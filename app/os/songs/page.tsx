...
export default function SongsPage() {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const payload = {
      ...
      releasePotential: form.releasePotential,
      notes: form.notes,
    ...
    <div>
              ...
              <div key={stage} className="w-72 shrink-0">
                ...
                <div className="space-y-3">
                  {stageSongs.map((song) => (
                    <div
                      key={song.id}
                      className="rounded-2xl border border-[#2a2a2a] bg-[#111111] p-4 transition-colors hover:border-[#3a3a3a]"
                    >
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "Edit Song" : "New Song"}>
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <FormInput label="Vibe" value={form.vibe} onChange={(e) => setForm({ ...form, vibe: e.target.value })} />
            <FormInput label="Emotional theme" value={form.theme} onChange={(e) => setForm({ ...form, theme: e.target.value })} />
          </div>
          <div className="grid grid-cols-3 gap-3">
            <FormInput label="Genre" value={form.genre} onChange={(e) => setForm({ ...form, genre: e.target.value })} />
            <FormInput label="BPM" type="number" value={form.bpm} onChange={(e) => setForm({ ...form, bpm: e.target.value })} />
            <FormInput label="Key" value={form.key} onChange={(e) => setForm({ ...form, key: e.target.value })} />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <FormInput label="Collaborators" value={form.collaborators} onChange={(e) => setForm({ ...form, collaborators: e.target.value })} />
            <FormInput label="Producer" value={form.producer} onChange={(e) => setForm({ ...form, producer: e.target.value })} />
          </div>
          <FormInput label="Next action" value={form.nextAction} onChange={(e) => setForm({ ...form, nextAction: e.target.value })} />
          <Select label="Release potential" value={form.releasePotential} onChange={(e) => setForm({ ...form, releasePotential: e.target.value as Priority })}>
            <option value="High">High</option>
            <option value="Medium">Medium</option>
            <option value="Low">Low</option>
          </Select>
          <TextArea label="Notes" value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} />
          <FormInput label="File links" value={form.fileLinks} onChange={(e) => setForm({ ...form, fileLinks: e.target.value })} />
