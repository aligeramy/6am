"use client";

import { create } from "zustand";
import { persist } from "zustand/middleware";
import {
  BrandNote,
  ContentItem,
  NotTodayItem,
  OSState,
  Release,
  Song,
  Task,
  VaultItem,
  WeeklyReview,
} from "@/types/os";
import { buildSeedData } from "./seed";

function uid(prefix: string) {
  return `${prefix}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
}

interface OSActions {
  // songs
  addSong: (song: Omit<Song, "id" | "createdAt" | "updatedAt">) => void;
  updateSong: (id: string, patch: Partial<Song>) => void;
  deleteSong: (id: string) => void;

  // releases
  addRelease: (release: Omit<Release, "id" | "createdAt" | "updatedAt">) => void;
  updateRelease: (id: string, patch: Partial<Release>) => void;
  deleteRelease: (id: string) => void;
  toggleChecklistItem: (
    releaseId: string,
    list: "preReleaseChecklist" | "releaseDayChecklist" | "postReleaseChecklist",
    itemId: string
  ) => void;

  // content
  addContent: (item: Omit<ContentItem, "id" | "createdAt" | "updatedAt">) => void;
  updateContent: (id: string, patch: Partial<ContentItem>) => void;
  deleteContent: (id: string) => void;

  // vault
  addVaultItem: (item: Omit<VaultItem, "id" | "createdAt" | "updatedAt">) => void;
  updateVaultItem: (id: string, patch: Partial<VaultItem>) => void;
  deleteVaultItem: (id: string) => void;

  // brand
  updateBrandNote: (id: string, patch: Partial<BrandNote>) => void;

  // weekly review
  addWeeklyReview: (review: Omit<WeeklyReview, "id" | "createdAt" | "updatedAt">) => void;
  updateWeeklyReview: (id: string, patch: Partial<WeeklyReview>) => void;
  deleteWeeklyReview: (id: string) => void;

  // tasks
  addTask: (task: Omit<Task, "id" | "createdAt" | "updatedAt">) => void;
  updateTask: (id: string, patch: Partial<Task>) => void;
  deleteTask: (id: string) => void;

  // not today
  addNotToday: (text: string) => void;
  updateNotToday: (id: string, text: string) => void;
  deleteNotToday: (id: string) => void;

  // today priority
  setTodayPriority: (text: string) => void;

  // import/export
  exportData: () => string;
  importData: (json: string) => boolean;
  resetToSeed: () => void;
}

export type OSStore = OSState & OSActions;

const seed = buildSeedData();

export const useOSStore = create<OSStore>()(
  persist(
    (set, get) => ({
      ...seed,

      addSong: (song) =>
        set((state) => ({
          songs: [
            ...state.songs,
            { ...song, id: uid("song"), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
          ],
        })),
      updateSong: (id, patch) =>
        set((state) => ({
          songs: state.songs.map((s) =>
            s.id === id ? { ...s, ...patch, updatedAt: new Date().toISOString() } : s
          ),
        })),
      deleteSong: (id) =>
        set((state) => ({ songs: state.songs.filter((s) => s.id !== id) })),

      addRelease: (release) =>
        set((state) => ({
          releases: [
            ...state.releases,
            { ...release, id: uid("release"), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
          ],
        })),
      updateRelease: (id, patch) =>
        set((state) => ({
          releases: state.releases.map((r) =>
            r.id === id ? { ...r, ...patch, updatedAt: new Date().toISOString() } : r
          ),
        })),
      deleteRelease: (id) =>
        set((state) => ({ releases: state.releases.filter((r) => r.id !== id) })),
      toggleChecklistItem: (releaseId, list, itemId) =>
        set((state) => ({
          releases: state.releases.map((r) => {
            if (r.id !== releaseId) return r;
            return {
              ...r,
              [list]: r[list].map((item) =>
                item.id === itemId ? { ...item, done: !item.done } : item
              ),
              updatedAt: new Date().toISOString(),
            };
          }),
        })),

      addContent: (item) =>
        set((state) => ({
          content: [
            ...state.content,
            { ...item, id: uid("content"), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
          ],
        })),
      updateContent: (id, patch) =>
        set((state) => ({
          content: state.content.map((c) =>
            c.id === id ? { ...c, ...patch, updatedAt: new Date().toISOString() } : c
          ),
        })),
      deleteContent: (id) =>
        set((state) => ({ content: state.content.filter((c) => c.id !== id) })),

      addVaultItem: (item) =>
        set((state) => ({
          vault: [
            ...state.vault,
            { ...item, id: uid("vault"), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
          ],
        })),
      updateVaultItem: (id, patch) =>
        set((state) => ({
          vault: state.vault.map((v) =>
            v.id === id ? { ...v, ...patch, updatedAt: new Date().toISOString() } : v
          ),
        })),
      deleteVaultItem: (id) =>
        set((state) => ({ vault: state.vault.filter((v) => v.id !== id) })),

      updateBrandNote: (id, patch) =>
        set((state) => ({
          brand: state.brand.map((b) =>
            b.id === id ? { ...b, ...patch, updatedAt: new Date().toISOString() } : b
          ),
        })),

      addWeeklyReview: (review) =>
        set((state) => ({
          weeklyReviews: [
            ...state.weeklyReviews,
            { ...review, id: uid("review"), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
          ],
        })),
      updateWeeklyReview: (id, patch) =>
        set((state) => ({
          weeklyReviews: state.weeklyReviews.map((w) =>
            w.id === id ? { ...w, ...patch, updatedAt: new Date().toISOString() } : w
          ),
        })),
      deleteWeeklyReview: (id) =>
        set((state) => ({ weeklyReviews: state.weeklyReviews.filter((w) => w.id !== id) })),

      addTask: (task) =>
        set((state) => ({
          tasks: [
            ...state.tasks,
            { ...task, id: uid("task"), createdAt: new Date().toISOString(), updatedAt: new Date().toISOString() },
          ],
        })),
      updateTask: (id, patch) =>
        set((state) => ({
          tasks: state.tasks.map((t) =>
            t.id === id ? { ...t, ...patch, updatedAt: new Date().toISOString() } : t
          ),
        })),
      deleteTask: (id) =>
        set((state) => ({ tasks: state.tasks.filter((t) => t.id !== id) })),

      addNotToday: (text) =>
        set((state) => ({
          notToday: [...state.notToday, { id: uid("nt"), text }],
        })),
      updateNotToday: (id, text) =>
        set((state) => ({
          notToday: state.notToday.map((n) => (n.id === id ? { ...n, text } : n)),
        })),
      deleteNotToday: (id) =>
        set((state) => ({ notToday: state.notToday.filter((n) => n.id !== id) })),

      setTodayPriority: (text) => set({ todayPriority: text }),

      exportData: () => {
        const state = get();
        const data: OSState = {
          songs: state.songs,
          releases: state.releases,
          content: state.content,
          vault: state.vault,
          brand: state.brand,
          weeklyReviews: state.weeklyReviews,
          tasks: state.tasks,
          notToday: state.notToday,
          todayPriority: state.todayPriority,
        };
        return JSON.stringify(data, null, 2);
      },
      importData: (json) => {
        try {
          const data = JSON.parse(json) as Partial<OSState>;
          if (!data || typeof data !== "object") return false;
          set((state) => ({
            songs: data.songs ?? state.songs,
            releases: data.releases ?? state.releases,
            content: data.content ?? state.content,
            vault: data.vault ?? state.vault,
            brand: data.brand ?? state.brand,
            weeklyReviews: data.weeklyReviews ?? state.weeklyReviews,
            tasks: data.tasks ?? state.tasks,
            notToday: data.notToday ?? state.notToday,
            todayPriority: data.todayPriority ?? state.todayPriority,
          }));
          return true;
        } catch {
          return false;
        }
      },
      resetToSeed: () => set(buildSeedData()),
    }),
    {
      name: "6am-os-storage",
    }
  )
);
